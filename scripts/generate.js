...
// 1. drive.js에서 getDriveClient, uploadToDrive와 함께 신규 추가한 uploadOrReplace를 가져옵니다.
const { getDriveClient, uploadToDrive, uploadOrReplace } = require("./drive");
admin.initializeApp({
...
async function main() {
  ...
  // 2. Google Drive 클라이언트를 생성하여 변수에 저장합니다.
  const drive = getDriveClient();
  // 3. 기존 코드에 있던 정의되지 않은 변수(buffer, filename) 사용 호출은 에러를 유발하므로 주석 처리합니다.
  // const fileId = await uploadToDrive(buffer, filename, "image/png");
  ...
  for (const col of imageTargets) {
    for (const item of gameData[col]) {
      ...
      // 4. drive 객체와 uploadOrReplace 함수를 사용하여 이미지 업로드를 수행합니다.
